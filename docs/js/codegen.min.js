...
out(output_if_difference_only=false,include_comment=false,custom_style=null){if(output_if_difference_only&&this.before===this.after){return'';}
let texts=[];if(include_comment&&this.comment!==null){const formattedComment=String(this.comment).split('\n').map(line=>`# ${line}`).join('\n');texts.push(formattedComment);}
...
out_display(override_value=null){let value=override_value!==null?override_value:this.after;if(this.partial_func&&typeof this.partial_func==='function'){value=this.partial_func(value);}else if(typeof value==='number'){value=value.toFixed(_FLOAT_PRECISION);value=value.replace(/(\.\d*?[1-9])0+$/,'$1').replace(/\.0+$/,'').replace(/\.$/,'.0');}
...
_DB_ASYNC_CPU_PROFILE={'max_worker_processes':{'tune_op':(group_cache,global_cache,options,response)=>cap_value(Math.ceil(options.vcpu*1.5)+2,4,512),'default':8,},'max_parallel_workers':{'tune_op':(group_cache,global_cache,options,response)=>Math.min(cap_value(Math.ceil(options.vcpu*1.25),4,512),group_cache['max_worker_processes']),'default':8,},'max_parallel_workers_per_gather':{'tune_op':(group_cache,global_cache,options,response)=>Math.min(cap_value(Math.ceil(options.vcpu/2.5),2,32),group_cache['max_parallel_workers']),'default':2,},'max_parallel_maintenance_workers':{'tune_op':(group_cache,global_cache,options,response)=>Math.min(cap_value(Math.ceil(options.vcpu/2),2,32),group_cache['max_parallel_workers']),'default':2,},'min_parallel_table_scan_size':{'instructions':{'medium_default':16*Mi,'large_default':24*Mi,'mall_default':32*Mi,'bigt_default':32*Mi,},'default':8*Mi,'partial_func':(value)=>`${Math.floor(value / DB_PAGE_SIZE) * Math.floor(DB_PAGE_SIZE / Ki)}kB`,},'min_parallel_index_scan_size':{'tune_op':(group_cache,global_cache,options,response)=>Math.max(group_cache['min_parallel_table_scan_size']/16,512*Ki),'default':512*Ki,'partial_func':(value)=>`${Math.floor(value / DB_PAGE_SIZE) * Math.floor(DB_PAGE_SIZE / Ki)}kB`,},}
_DB_WAL_PROFILE={'wal_level':{'default':'replica',},'synchronous_commit':{'default':'on',},'full_page_writes':{'default':'on',},'fsync':{'default':'on',},'wal_compression':{'default':'pglz',},'wal_init_zero':{'default':'on',},'wal_recycle':{'default':'on',},'wal_log_hints':{'default':'on',},'wal_writer_delay':{'instructions':{"mini_default":K10,},'default':200,'partial_func':(value)=>`${value}ms`,},'wal_writer_flush_after':{'default':1*Mi,'partial_func':(value)=>`${Math.floor(value / Mi)}MB`,},'checkpoint_timeout':{'instructions':{'mini_default':30*MINUTE,'mall_default':10*MINUTE,'bigt_default':10*MINUTE,},'default':15*MINUTE,'hardware_scope':'overall','partial_func':(value)=>`${Math.floor(value / MINUTE)}min`,},'checkpoint_flush_after':{'default':256*Ki,'partial_func':(value)=>`${Math.floor(value / Ki)}kB`,},'checkpoint_completion_target':{'default':0.9,},'checkpoint_warning':{'default':30,'partial_func':(value)=>`${value}s`,},'min_wal_size':{'tune_op':(group_cache,global_cache,options,response)=>10*options.tuning_kwargs.wal_segment_size,'default':10*BASE_WAL_SEGMENT_SIZE,'partial_func':(value)=>`${Math.floor(value / Mi)}MB`,},'max_wal_size':{'instructions':{'mini_default':2*Gi,'medium_default':4*Gi,'large_default':8*Gi,'mall_default':16*Gi,'bigt_default':32*Gi,},'default':8*Gi,'partial_func':(value)=>`${Math.floor(value / Mi)}MB`,},'wal_buffers':{'tune_op':(group_cache,global_cache,options,response)=>_CalcWalBuffers(group_cache,global_cache,options,response,Math.floor(BASE_WAL_SEGMENT_SIZE/2),BASE_WAL_SEGMENT_SIZE*16),'default':2*BASE_WAL_SEGMENT_SIZE,'hardware_scope':'mem',},'archive_mode':{'default':'on',},'archive_timeout':{'instructions':{'mini_default':1*HOUR,'mall_default':30*MINUTE,'bigt_default':30*MINUTE,},'default':45*MINUTE,'hardware_scope':'overall','partial_func':(value)=>`${value}s`,},}
...
const DB0_CONFIG_PROFILE={"connection":[PG_SCOPE.CONNECTION,_DB_CONN_PROFILE,{hardware_scope:'cpu'}],"memory":[PG_SCOPE.MEMORY,_DB_RESOURCE_PROFILE,{hardware_scope:'mem'}],"maintenance":[PG_SCOPE.MAINTENANCE,_DB_VACUUM_PROFILE,{hardware_scope:'overall'}],"background_writer":[PG_SCOPE.OTHERS,_DB_BGWRITER_PROFILE,{hardware_scope:'disk'}],"asynchronous_disk":[PG_SCOPE.OTHERS,_DB_ASYNC_DISK_PROFILE,{hardware_scope:'disk'}],"asynchronous_cpu":[PG_SCOPE.OTHERS,_DB_ASYNC_CPU_PROFILE,{hardware_scope:'cpu'}],"wal":[PG_SCOPE.ARCHIVE_RECOVERY_BACKUP_RESTORE,_DB_WAL_PROFILE,{hardware_scope:'disk'}],"query":[PG_SCOPE.QUERY_TUNING,_DB_QUERY_PROFILE,{hardware_scope:'cpu'}],"log":[PG_SCOPE.LOGGING,_DB_LOG_PROFILE,{hardware_scope:'disk'}],"replication":[PG_SCOPE.ARCHIVE_RECOVERY_BACKUP_RESTORE,_DB_REPLICATION_PROFILE,{hardware_scope:'disk'}],"timeout":[PG_SCOPE.OTHERS,_DB_TIMEOUT_PROFILE,{hardware_scope:'overall'}],"lib":[PG_SCOPE.EXTRA,_DB_LIB_PROFILE,{hardware_scope:'overall'}],};merge_extra_info_to_profile(DB0_CONFIG_PROFILE);type_validation(DB0_CONFIG_PROFILE);const DB13_CONFIG_PROFILE={...DB0_CONFIG_PROFILE};const _DB14_TIMEOUT_PROFILE={'idle_session_timeout':{"default":0,"partial_func":value=>`${value}s`,},};const _DB14_QUERY_PROFILE={'track_wal_io_timing':{"default":'on',},};const _DB14_VACUUM_PROFILE={'vacuum_failsafe_age':{"default":1600000000,},'vacuum_multixact_failsafe_age':{"default":1600000000,}};const DB14_CONFIG_MAPPING={timeout:[PG_SCOPE.OTHERS,_DB14_TIMEOUT_PROFILE,{hardware_scope:'overall'}],query:[PG_SCOPE.QUERY_TUNING,_DB14_QUERY_PROFILE,{hardware_scope:'overall'}],maintenance:[PG_SCOPE.MAINTENANCE,_DB14_VACUUM_PROFILE,{hardware_scope:'overall'}],};merge_extra_info_to_profile(DB14_CONFIG_MAPPING);type_validation(DB14_CONFIG_MAPPING);let DB14_CONFIG_PROFILE={...DB0_CONFIG_PROFILE};if(Object.keys(DB14_CONFIG_MAPPING).length>0){for(const[key,value]of Object.entries(DB14_CONFIG_MAPPING)){if(key in DB14_CONFIG_PROFILE){let src=DB14_CONFIG_PROFILE[key][1];let dst=value[1];for(const[k,v]of Object.entries(dst)){src[k]=v;}}}
rewrite_items(DB14_CONFIG_PROFILE);}
const _DB15_LOG_PROFILE={"log_startup_progress_interval":{"default":K10,"partial_func":value=>`${value}s`,},};const _DB15_TIMEOUT_PROFILE={'idle_session_timeout':{"default":0,"partial_func":value=>`${value}s`,},};const _DB15_QUERY_PROFILE={'track_wal_io_timing':{'default':'on',},};const _DB15_VACUUM_PROFILE={'vacuum_failsafe_age':{'default':1600000000,},'vacuum_multixact_failsafe_age':{'default':1600000000,}};const DB15_CONFIG_MAPPING={log:[PG_SCOPE.LOGGING,_DB15_LOG_PROFILE,{hardware_scope:'disk'}],timeout:[PG_SCOPE.OTHERS,_DB15_TIMEOUT_PROFILE,{hardware_scope:'overall'}],query:[PG_SCOPE.QUERY_TUNING,_DB15_QUERY_PROFILE,{hardware_scope:'overall'}],maintenance:[PG_SCOPE.MAINTENANCE,_DB15_VACUUM_PROFILE,{hardware_scope:'overall'}],};merge_extra_info_to_profile(DB15_CONFIG_MAPPING);type_validation(DB15_CONFIG_MAPPING);let DB15_CONFIG_PROFILE={...DB0_CONFIG_PROFILE};if(Object.keys(DB15_CONFIG_MAPPING).length>0){for(const[key,value]of Object.entries(DB15_CONFIG_MAPPING)){if(key in DB15_CONFIG_PROFILE){let src=DB15_CONFIG_PROFILE[key][1];let dst=value[1];for(const[k,v]of Object.entries(dst)){src[k]=v;}}}
rewrite_items(DB15_CONFIG_PROFILE);}
const _DB16_LOG_PROFILE={"log_startup_progress_interval":{"default":K10,"partial_func":value=>`${value}s`,},};const _DB16_VACUUM_PROFILE={'vacuum_buffer_usage_limit':{"tune_op":(group_cache,global_cache,options,response)=>realign_value(cap_value(Math.floor(group_cache['maintenance_work_mem']/16),2*Mi,16*Gi),DB_PAGE_SIZE)[options.align_index],"default":2*Mi,"hardware_scope":"mem","partial_func":value=>`${Math.floor(value / Mi)}MB`,},"vacuum_failsafe_age":{"default":1600000000,},"vacuum_multixact_failsafe_age":{"default":1600000000,}};const _DB16_WAL_PROFILE={"wal_compression":{"default":"zstd",},};const _DB16_TIMEOUT_PROFILE={"idle_session_timeout":{"default":0,"partial_func":value=>`${value}s`,},};const _DB16_QUERY_PROFILE={"track_wal_io_timing":{"default":'on',},};const DB16_CONFIG_MAPPING={log:[PG_SCOPE.LOGGING,_DB16_LOG_PROFILE,{hardware_scope:'disk'}],timeout:[PG_SCOPE.OTHERS,_DB16_TIMEOUT_PROFILE,{hardware_scope:'overall'}],query:[PG_SCOPE.QUERY_TUNING,_DB16_QUERY_PROFILE,{hardware_scope:'overall'}],maintenance:[PG_SCOPE.MAINTENANCE,_DB16_VACUUM_PROFILE,{hardware_scope:'overall'}],wal:[PG_SCOPE.ARCHIVE_RECOVERY_BACKUP_RESTORE,_DB16_WAL_PROFILE,{hardware_scope:'disk'}],};merge_extra_info_to_profile(DB16_CONFIG_MAPPING);type_validation(DB16_CONFIG_MAPPING);let DB16_CONFIG_PROFILE={...DB0_CONFIG_PROFILE}
...
const _DB17_LOG_PROFILE={"log_startup_progress_interval":{"default":K10,"partial_func":value=>`${value}s`,}};const _DB17_VACUUM_PROFILE={"vacuum_buffer_usage_limit":{"tune_op":(group_cache,global_cache,options,response)=>realign_value(cap_value(Math.floor(group_cache['shared_buffers']/16),2*Mi,16*Gi),DB_PAGE_SIZE)[options.align_index],"default":2*Mi,"hardware_scope":"mem","partial_func":value=>`${Math.floor(value / Mi)}MB`,},"vacuum_failsafe_age":{"default":1600000000,},"vacuum_multixact_failsafe_age":{"default":1600000000,}};const _DB17_WAL_PROFILE={"wal_compression":{"default":"zstd",},"summarize_wal":{"default":"on",},"wal_summary_keep_time":{"default":Math.floor(30*DAY/MINUTE),"partial_func":value=>`${Math.floor(value / MINUTE)}min`,},};const _DB17_TIMEOUT_PROFILE={"idle_session_timeout":{"default":0,"partial_func":value=>`${value}s`,},};const _DB17_QUERY_PROFILE={"track_wal_io_timing":{"default":'on',},};const DB17_CONFIG_MAPPING={log:[PG_SCOPE.LOGGING,_DB17_LOG_PROFILE,{hardware_scope:'disk'}],timeout:[PG_SCOPE.OTHERS,_DB17_TIMEOUT_PROFILE,{hardware_scope:'overall'}],query:[PG_SCOPE.QUERY_TUNING,_DB17_QUERY_PROFILE,{hardware_scope:'overall'}],maintenance:[PG_SCOPE.MAINTENANCE,_DB17_VACUUM_PROFILE,{hardware_scope:'overall'}],wal:[PG_SCOPE.ARCHIVE_RECOVERY_BACKUP_RESTORE,_DB17_WAL_PROFILE,{hardware_scope:'disk'}],};merge_extra_info_to_profile(DB17_CONFIG_MAPPING);type_validation(DB17_CONFIG_MAPPING);let DB17_CONFIG_PROFILE={...DB0_CONFIG_PROFILE}
...
function wal_time(wal_buffers,data_amount_ratio,wal_segment_size,wal_writer_delay_in_ms,wal_throughput){console.debug('Estimate the time required to flush the full WAL buffers to disk');const data_amount=Math.floor(wal_buffers*data_amount_ratio);const num_wal_files_required=Math.floor(data_amount/wal_segment_size)+1;const rotate_time_in_ms=num_wal_files_required*_FILE_ROTATION_TIME_MS;const write_time_in_ms=(data_amount/Mi)/wal_throughput*K10;let delay_time=0;if(data_amount_ratio>1){let num_delay=Math.floor(data_amount_ratio);const fractional=data_amount_ratio-num_delay;if(fractional===0){num_delay-=1;}
...
class PG_TUNE_REQUEST{constructor(options){this.options=options.options||{};this.include_comment=options.include_comment||false;this.custom_style=options.custom_style||null;this.backup_settings=options.backup_settings||false;this.analyze_with_full_connection_use=options.analyze_with_full_connection_use||false;this.ignore_non_performance_setting=options.ignore_non_performance_setting||false;this.output_format=options.output_format||'file';}}
...
_generate_content_as_file(target,request,backup_settings=true,exclude_names=null){let content=[target.disclaimer(),'\n'];if(backup_settings){content.push(`# User Options: ${JSON.stringify(request.options)}\n`);}
for(const[scope,items]of Object.entries(this.outcome[target])){content.push(`## ===== SCOPE: ${scope} ===== \n`);for(const[item_name,item]of Object.entries(items)){if(exclude_names===null||!exclude_names.has(item_name)){content.push(item.out(request.include_comment,request.custom_style));content.push(request.include_comment?'\n\n':'\n');}}
content.push('\n'+(request.include_comment?'\n\n':''));}
return content.join('');}
_generate_content_as_response(target,exclude_names=null,output_format='conf'){let content={};for(const[_,items]of Object.entries(this.outcome[target])){for(const[item_name,item]of Object.entries(items)){if(exclude_names===null||!exclude_names.has(item_name)){content[item_name]=item.out_display(null);}}}
if(output_format==='conf'){return Object.entries(content).map(([k,v])=>`${k} = ${v}`).join('\n');}
return content;}
generate_content(target,request,exclude_names=null,backup_settings=true,output_format='conf'){if(exclude_names!==null&&Array.isArray(exclude_names)){exclude_names=new Set(exclude_names);}
if(output_format==='file'){return this._generate_content_as_file(target,request,backup_settings,exclude_names);}else if(['json','conf'].includes(output_format)){return this._generate_content_as_response(target,exclude_names,output_format);}else{throw new Error(`Invalid output format: ${output_format}. Expected one of "json", "conf", "file".`);}}
...
const effective_cache_size=managed_cache['effective_cache_size'];const wal_throughput=options.wal_spec.perf()[0];const wal10=wal_time(wal_buffers,1.0,_kwargs.wal_segment_size,managed_cache['wal_writer_delay'],wal_throughput);const wal15=wal_time(wal_buffers,1.5,_kwargs.wal_segment_size,managed_cache['wal_writer_delay'],wal_throughput);const wal20=wal_time(wal_buffers,2.0,_kwargs.wal_segment_size,managed_cache['wal_writer_delay'],wal_throughput);let real_autovacuum_work_mem=managed_cache['autovacuum_work_mem'];if(real_autovacuum_work_mem===-1){real_autovacuum_work_mem=managed_cache['maintenance_work_mem'];}
...
if(PG_DISK_SIZING.matchDiskSeries(data_iops,RANDOM_IOPS,`hdd`,`weak`)){after_random_page_cost=3.25}else if(PG_DISK_SIZING.matchDiskSeries(data_iops,RANDOM_IOPS,`hdd`,`strong`)){after_random_page_cost=2.60}else if(PG_DISK_SIZING.matchDiskSeries(data_iops,RANDOM_IOPS,`san`,`weak`)){after_random_page_cost=2.00}else if(PG_DISK_SIZING.matchDiskSeries(data_iops,RANDOM_IOPS,`san`,`strong`)){after_random_page_cost=1.50}else if(PG_DISK_SIZING.matchOneDisk(data_iops,RANDOM_IOPS,PG_DISK_SIZING.SSDv1)){after_random_page_cost=1.25}else if(PG_DISK_SIZING.matchOneDisk(data_iops,RANDOM_IOPS,PG_DISK_SIZING.SSDv2)){after_random_page_cost=1.20}else if(PG_DISK_SIZING.matchOneDisk(data_iops,RANDOM_IOPS,PG_DISK_SIZING.SSDv3)){after_random_page_cost=1.15}else if(PG_DISK_SIZING.matchOneDisk(data_iops,RANDOM_IOPS,PG_DISK_SIZING.SSDv4)){after_random_page_cost=1.10}else if(PG_DISK_SIZING.matchOneDisk(data_iops,RANDOM_IOPS,PG_DISK_SIZING.SSDv5)){after_random_page_cost=1.05}
...
while(transaction_loss_time<=wal_time(current_wal_buffers,data_amount_ratio_input,_kwargs.wal_segment_size,after_wal_writer_delay,wal_tput)['total_time']){current_wal_buffers-=decay_rate}
...
function _build_request_from_backend(data){return new PG_TUNE_REQUEST({'options':_build_options_from_backend(data.options),'include_comment':data.include_comment??false,'custom_style':data.custom_style??null,'backup_settings':data.backup_settings??false,'analyze_with_full_connection_use':data.analyze_with_full_connection_use??false,'ignore_non_performance_setting':data.ignore_non_performance_setting??true,'output_format':data.output_format??'file',})}
function _build_request_from_html(){let alter_style=_get_checkbox_element(`alter_style`)??false;let custom_style=!alter_style?null:'ALTER SYSTEM SET $1 = $2;'
return{'options':_build_options_from_html(),'include_comment':_get_checkbox_element(`include_comment`)??false,'custom_style':custom_style,'backup_settings':_get_checkbox_element(`backup_settings`)??false,'analyze_with_full_connection_use':_get_checkbox_element(`analyze_with_full_connection_use`)??false,'ignore_non_performance_setting':_get_checkbox_element(`ignore_non_performance_setting`)??true,'output_format':_get_text_element(`output_format`)??'file',}}
function web_optimize(request){let response=new PG_TUNE_RESPONSE();let items={13:DB13_CONFIG_PROFILE,14:DB14_CONFIG_PROFILE,15:DB15_CONFIG_PROFILE,16:DB16_CONFIG_PROFILE,17:DB17_CONFIG_PROFILE,}
...
console.log(request);console.log(request.options);Optimize(request,response,PGTUNER_SCOPE.DATABASE_CONFIG,tuning_items);if(request.options.enable_database_correction_tuning){correction_tune(request,response);}
let exclude_names=['archive_command','restore_command','archive_cleanup_command','recovery_end_command','log_directory',];if(request.ignore_non_performance_setting){exclude_names.push('deadlock_timeout','transaction_timeout','idle_session_timeout','log_autovacuum_min_duration','log_checkpoints','log_connections','log_disconnections','log_duration','log_error_verbosity','log_line_prefix','log_lock_waits','log_recovery_conflict_waits','log_statement','log_replication_commands','log_min_error_statement','log_startup_progress_interval')}
...
const content=response.generate_content(PGTUNER_SCOPE.DATABASE_CONFIG,request,exclude_names,request.backup_settings,request.output_format);const mem_report=response.report(request.options,request.analyze_with_full_connection_use,false)[0];return{'content':content,'mem_report':mem_report,'response':response,}}