...
let after_bgwriter_delay=Math.floor(Math.max(150,300-30*request.options.workload_profile.num()-5*data_iops/K10));_ApplyItmTune('bgwriter_delay',after_bgwriter_delay,PG_SCOPE.OTHERS,response);let bg_io_per_cycle=0.060;if(request.options.workload_type===PG_WORKLOAD.VECTOR){bg_io_per_cycle=0.035;}else if(request.options.workload_type===PG_WORKLOAD.TSR_IOT){bg_io_per_cycle=0.075;}
const after_bgwriter_lru_maxpages=cap_value(30*request.options.workload_profile.num()+data_iops*cap_value(bg_io_per_cycle,1e-3,1e-1),100+30*request.options.workload_profile.num(),4000);_ApplyItmTune('bgwriter_lru_maxpages',after_bgwriter_lru_maxpages,PG_SCOPE.OTHERS,response);console.log(`Start tuning the autovacuum of the PostgreSQL database server based on the database workload.`)
...
_ApplyItmTune('checkpoint_warning',Math.floor(after_checkpoint_timeout*1.25*(1-managed_cache['checkpoint_completion_target'])),PG_SCOPE.ARCHIVE_RECOVERY_BACKUP_RESTORE,response)
return null;}
