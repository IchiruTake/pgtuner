...
class PG_TUNE_USR_KWARGS{constructor(options={}){this.user_max_connections=options.user_max_connections??0;this.cpu_to_connection_scale_ratio=options.cpu_to_connection_scale_ratio??5;this.cpu_to_parallel_scale_ratio=options.cpu_to_parallel_scale_ratio??2;this.superuser_reserved_connections_scale_ratio=options.superuser_reserved_connections_scale_ratio??1.5;this.single_memory_connection_overhead=options.single_memory_connection_overhead??(5*Mi);this.memory_connection_to_dedicated_os_ratio=options.memory_connection_to_dedicated_os_ratio??0.7;this.effective_cache_size_available_ratio=options.effective_cache_size_available_ratio??0.985;this.shared_buffers_ratio=options.shared_buffers_ratio??0.25;this.max_work_buffer_ratio=options.max_work_buffer_ratio??0.125;this.effective_connection_ratio=options.effective_connection_ratio??0.75;this.temp_buffers_ratio=options.temp_buffers_ratio??0.25;this.max_normal_memory_usage=options.max_normal_memory_usage??0.55;this.mem_pool_tuning_ratio=options.mem_pool_tuning_ratio??0.45;this.hash_mem_usage_level=options.hash_mem_usage_level??-3;this.mem_pool_parallel_estimate=options.mem_pool_parallel_estimate??true;this.max_query_length_in_bytes=options.max_query_length_in_bytes??(2*Ki);this.max_runtime_ms_to_log_slow_query=options.max_runtime_ms_to_log_slow_query??(2*K10);this.max_runtime_ratio_to_explain_slow_query=options.max_runtime_ratio_to_explain_slow_query??1.5;this.wal_segment_size=options.wal_segment_size??BASE_WAL_SEGMENT_SIZE;this.min_wal_size_ratio=options.min_wal_size_ratio??0.025;this.max_wal_size_ratio=options.max_wal_size_ratio??0.04;this.wal_keep_size_ratio=options.wal_keep_size_ratio??0.04;this.autovacuum_utilization_ratio=options.autovacuum_utilization_ratio??0.80;this.vacuum_safety_level=options.vacuum_safety_level??2;}}
function _EstimateKernelInUseMemory(kernel_memory,operating_system){if(kernel_memory===-1){kernel_memory=768;if(operating_system==="containerd"||operating_system==="macos"){kernel_memory=64;}else if(operating_system==="windows"){kernel_memory=2048;}else if(operating_system==="PaaS"){kernel_memory=0;}}
return kernel_memory;}
function _EstimateMonitoringInUseMemory(monitoring_memory,operating_system){if(monitoring_memory===-1){monitoring_memory=256;if(operating_system==="containerd"){monitoring_memory=64;}else if(operating_system==="PaaS"){monitoring_memory=0;}}
return monitoring_memory;}
function _RamCalculator(){let kernel_memory=_EstimateKernelInUseMemory(document.getElementById("base_kernel_memory_usage_in_mib").value*1,document.getElementById("operating_system").value)
let monitoring_memory=_EstimateMonitoringInUseMemory(document.getElementById("base_monitoring_memory_usage_in_mib").value*1,document.getElementById("operating_system").value)
let total_ram=document.getElementById("total_ram_in_gib").value*1024;const final_ram=total_ram-kernel_memory-monitoring_memory;const postgresql_ram_available_block=document.getElementById("total_usable_ram");postgresql_ram_available_block.value=final_ram;return final_ram;}
...
this.base_monitoring_memory_usage=_EstimateMonitoringInUseMemory(this.base_monitoring_memory_usage>0?this.base_monitoring_memory_usage/Mi:this.base_monitoring_memory_usage,this.operating_system)*Mi;this.base_kernel_memory_usage=_EstimateKernelInUseMemory(this.base_kernel_memory_usage>0?this.base_kernel_memory_usage/Mi:this.base_kernel_memory_usage,this.operating_system)*Mi;this.usable_ram=this.total_ram-this.base_kernel_memory_usage-this.base_monitoring_memory_usage
console.debug(`The usable RAM is ${bytesize_to_hr(this.usable_ram)}`);if(this.usable_ram<4*Gi){const _sign=(this.usable_ram>=0)?'+':'-';const _msg=`The usable RAM ${_sign}${bytesize_to_hr(this.usable_ram, false, ' ')} is less than 4 GiB. Tuning may not be accurate.`;console.warn(_msg);}
...
function _CalcWalBuffers(group_cache,global_cache,options,response,minimum,maximum){let shared_buffers=global_cache['shared_buffers'];let usable_ram_noswap=options.usable_ram;function fn(x){return Ki*(37.25*Math.log(x)+2)*0.90;}
...
_DB_BGWRITER_PROFILE={'bgwriter_delay':{'default':200,'hardware_scope':'overall','partial_func':(value)=>`${value}ms`,},'bgwriter_lru_maxpages':{'instructions':{'large_default':350,'mall_default':425,'bigt_default':500,},'default':300,},'bgwriter_lru_multiplier':{'default':2.0,},'bgwriter_flush_after':{'default':512*Ki,'partial_func':(value)=>`${Math.floor(value / Ki)}kB`,},}
_DB_ASYNC_DISK_PROFILE={'effective_io_concurrency':{'default':16,},'maintenance_io_concurrency':{'default':10,},'backend_flush_after':{'default':0,'partial_func':(value)=>`${Math.floor(value / Ki)}kB`,},}
_DB_ASYNC_CPU_PROFILE={'max_worker_processes':{'tune_op':(group_cache,global_cache,options,response)=>cap_value(Math.ceil(options.vcpu*(0.5+options.tuning_kwargs.cpu_to_parallel_scale_ratio))+2,4,512),'default':8,},'max_parallel_workers':{'tune_op':(group_cache,global_cache,options,response)=>Math.min(cap_value(Math.ceil(options.vcpu*options.tuning_kwargs.cpu_to_parallel_scale_ratio)+1,4,512),group_cache['max_worker_processes']),'default':8,},'max_parallel_workers_per_gather':{'tune_op':(group_cache,global_cache,options,response)=>Math.min(cap_value(Math.ceil(options.vcpu*(options.tuning_kwargs.cpu_to_parallel_scale_ratio-0.25)/3),2,32),Math.min(options.vcpu,group_cache['max_parallel_workers'])),'default':2,},'max_parallel_maintenance_workers':{'tune_op':(group_cache,global_cache,options,response)=>Math.min(cap_value(Math.ceil(options.vcpu*(options.tuning_kwargs.cpu_to_parallel_scale_ratio-0.25)/2.5),2,32),Math.min(options.vcpu,group_cache['max_parallel_workers'])),'default':2,},'min_parallel_table_scan_size':{'instructions':{'medium_default':16*Mi,'large_default':24*Mi,'mall_default':32*Mi,'bigt_default':32*Mi,},'default':8*Mi,'partial_func':(value)=>`${Math.floor(value / DB_PAGE_SIZE) * Math.floor(DB_PAGE_SIZE / Ki)}kB`,},'min_parallel_index_scan_size':{'tune_op':(group_cache,global_cache,options,response)=>Math.max(group_cache['min_parallel_table_scan_size']/16,512*Ki),'default':512*Ki,'partial_func':(value)=>`${Math.floor(value / DB_PAGE_SIZE) * Math.floor(DB_PAGE_SIZE / Ki)}kB`,},}
_DB_WAL_PROFILE={'wal_level':{'default':'replica',},'synchronous_commit':{'default':'on',},'full_page_writes':{'default':'on',},'fsync':{'default':'on',},'wal_compression':{'default':'pglz',},'wal_init_zero':{'default':'on',},'wal_recycle':{'default':'on',},'wal_log_hints':{'default':'on',},'wal_writer_delay':{'instructions':{"mini_default":K10,},'default':200,'partial_func':(value)=>`${value}ms`,},'wal_writer_flush_after':{'default':1*Mi,'partial_func':(value)=>`${Math.floor(value / Mi)}MB`,},'checkpoint_timeout':{'instructions':{'mini_default':30*MINUTE,'mall_default':10*MINUTE,'bigt_default':10*MINUTE,},'default':15*MINUTE,'hardware_scope':'overall','partial_func':(value)=>`${Math.floor(value / MINUTE)}min`,},'checkpoint_flush_after':{'default':256*Ki,'partial_func':(value)=>`${Math.floor(value / Ki)}kB`,},'checkpoint_completion_target':{'default':0.9,},'checkpoint_warning':{'default':30,'partial_func':(value)=>`${value}s`,},'min_wal_size':{'tune_op':(group_cache,global_cache,options,response)=>10*options.tuning_kwargs.wal_segment_size,'default':10*BASE_WAL_SEGMENT_SIZE,'partial_func':(value)=>`${Math.floor(value / Mi)}MB`,},'max_wal_size':{'instructions':{'mini_default':2*Gi,'medium_default':4*Gi,'large_default':8*Gi,'mall_default':16*Gi,'bigt_default':32*Gi,},'default':8*Gi,'partial_func':(value)=>`${Math.floor(value / Mi)}MB`,},'wal_buffers':{'tune_op':(group_cache,global_cache,options,response)=>_CalcWalBuffers(group_cache,global_cache,options,response,BASE_WAL_SEGMENT_SIZE,BASE_WAL_SEGMENT_SIZE*16),'default':2*BASE_WAL_SEGMENT_SIZE,'hardware_scope':'mem','partial_func':(value)=>`${Math.floor(value / DB_PAGE_SIZE) * Math.floor(DB_PAGE_SIZE / Ki)}kB`,},'archive_mode':{'default':'on',},'archive_timeout':{'instructions':{'mini_default':1*HOUR,'mall_default':30*MINUTE,'bigt_default':30*MINUTE,},'default':45*MINUTE,'hardware_scope':'overall','partial_func':(value)=>`${value}s`,},}
...
const _DB17_WAL_PROFILE={"summarize_wal":{"default":"on",},"wal_summary_keep_time":{"default":Math.floor(30*DAY/MINUTE),"partial_func":value=>`${Math.floor(value / MINUTE)}min`,},};const _DB17_TIMEOUT_PROFILE={"idle_session_timeout":{"default":0,"partial_func":value=>`${value}s`,},"transaction_timeout":{"default":0,"partial_func":value=>`${value}s`,},};const _DB17_ASYNC_DISK_PROFILE={"io_combine_limit":{"default":128*Ki,"partial_func":value=>`${Math.floor(value / DB_PAGE_SIZE) * Math.floor(DB_PAGE_SIZE / Ki)}kB`,},};const DB17_CONFIG_MAPPING={timeout:[PG_SCOPE.OTHERS,_DB17_TIMEOUT_PROFILE,{hardware_scope:'overall'}],wal:[PG_SCOPE.ARCHIVE_RECOVERY_BACKUP_RESTORE,_DB17_WAL_PROFILE,{hardware_scope:'overall'}],"asynchronous_disk":[PG_SCOPE.OTHERS,_DB17_ASYNC_DISK_PROFILE,{hardware_scope:'disk'}],};merge_extra_info_to_profile(DB17_CONFIG_MAPPING);type_validation(DB17_CONFIG_MAPPING);const DB17_CONFIG_PROFILE={}
for(const[key,value]of Object.entries(DB16_CONFIG_PROFILE)){DB17_CONFIG_PROFILE[key]=[value[0],{...value[1]},value[2]];}
...
const _DB18_ASYNC_DISK_PROFILE={"io_max_combine_limit":{"default":128*Ki,"partial_func":value=>`${Math.floor(value / DB_PAGE_SIZE) * Math.floor(DB_PAGE_SIZE / Ki)}kB`,},"io_max_concurrency":{"default":cap_value(-1,-1,1024)},"io_method":{"default":"worker",},"io_workers":{"default":cap_value(3,1,32),},};const _DB18_VACUUM_PROFILE={"autovacuum_vacuum_max_threshold":{"default":cap_value(100*M10,-1,2**31-1)},"autovacuum_worker_slots":{"default":cap_value(16,-1,2**18-1)},"vacuum_max_eager_freeze_failure_rate":{"default":cap_value(0.03,0.0,1.0)},"vacuum_truncate":{"default":'on'},};const _DB18_QUERY_PROFILE={"track_cost_delay_timing":{"default":'on',}};const _DB18_LOG_PROFILE={"log_lock_failure":{"default":'on',}};const _DB18_TIMEOUT_PROFILE={"idle_replication_slot_timeout":{"default":cap_value(0,0,35791394)},};const _DB18_REPLICATION_PROFILE={"max_active_replication_origins":{"default":cap_value(10,0,2**18-1)},};const DB18_CONFIG_MAPPING={"asynchronous_disk":[PG_SCOPE.OTHERS,_DB18_ASYNC_DISK_PROFILE,{hardware_scope:'disk'}],maintenance:[PG_SCOPE.MAINTENANCE,_DB18_VACUUM_PROFILE,{'hardware_scope':'overall'}],query:[PG_SCOPE.QUERY_TUNING,_DB18_QUERY_PROFILE,{hardware_scope:'overall'}],log:[PG_SCOPE.LOGGING,_DB18_LOG_PROFILE,{hardware_scope:'disk'}],timeout:[PG_SCOPE.OTHERS,_DB18_TIMEOUT_PROFILE,{hardware_scope:'overall'}],replication:[PG_SCOPE.ARCHIVE_RECOVERY_BACKUP_RESTORE,_DB18_REPLICATION_PROFILE,{hardware_scope:'cpu'}],};merge_extra_info_to_profile(DB18_CONFIG_MAPPING);type_validation(DB18_CONFIG_MAPPING);const DB18_CONFIG_PROFILE={}
for(const[key,value]of Object.entries(DB17_CONFIG_PROFILE)){DB18_CONFIG_PROFILE[key]=[value[0],{...value[1]},value[2]];}
...
const effective_cache_size=managed_cache['effective_cache_size'];const wal_throughput=options.wal_spec.perf()[0];const wal_writer_delay=managed_cache['wal_writer_delay']
const wal05=wal_time(wal_buffers,0.5,_kwargs.wal_segment_size,wal_writer_delay,wal_throughput,options,managed_cache['wal_init_zero']);const wal10=wal_time(wal_buffers,1.0,_kwargs.wal_segment_size,wal_writer_delay,wal_throughput,options,managed_cache['wal_init_zero']);const wal15=wal_time(wal_buffers,1.5,_kwargs.wal_segment_size,wal_writer_delay,wal_throughput,options,managed_cache['wal_init_zero']);const wal20=wal_time(wal_buffers,2.0,_kwargs.wal_segment_size,wal_writer_delay,wal_throughput,options,managed_cache['wal_init_zero']);let real_autovacuum_work_mem=managed_cache['autovacuum_work_mem'];if(real_autovacuum_work_mem===-1){real_autovacuum_work_mem=managed_cache['maintenance_work_mem'];}
...
if(request.options.operating_system!=='windows'){let after_checkpoint_flush_after=512*Ki
...
if(PG_DISK_SIZING.matchDiskSeries(data_iops,RANDOM_IOPS,'san','strong')){after_checkpoint_flush_after=768*Ki
after_bgwriter_flush_after=768*Ki}else if(PG_DISK_SIZING.matchDiskSeriesInRange(data_iops,RANDOM_IOPS,'ssd','nvme')){after_checkpoint_flush_after=1*Mi
...
if(request.options.workload_profile>=PG_SIZING.MALL){after_wal_writer_flush_after*=2}}
...
let after_bgwriter_delay=Math.floor(Math.max(200,Math.floor(400-30*request.options.workload_profile.num()-5*data_iops/K10)));_ApplyItmTune('bgwriter_delay',after_bgwriter_delay,PG_SCOPE.OTHERS,response);let bg_io_per_cycle=0.065;if(request.options.workload_type===PG_WORKLOAD.VECTOR){bg_io_per_cycle=0.035;}else if(request.options.workload_type===PG_WORKLOAD.TSR_IOT){bg_io_per_cycle=0.080;}
const after_bgwriter_lru_maxpages=cap_value(Math.floor(40*request.options.workload_profile.num()+data_iops*cap_value(bg_io_per_cycle,1e-3,1e-1)),100+30*request.options.workload_profile.num(),4000);_ApplyItmTune('bgwriter_lru_maxpages',after_bgwriter_lru_maxpages,PG_SCOPE.OTHERS,response);const _max_write_time=Math.floor(after_bgwriter_lru_maxpages/data_iops*K10);console.info(`The background writer is tuned to write at most ${after_bgwriter_lru_maxpages} pages per cycle with ${after_bgwriter_delay} ms delay. -> Resulting in maximum of ${_max_write_time} ms of WRITE time and peak utilization of ${(100 * _max_write_time / (_max_write_time + after_bgwriter_delay)).toFixed(2)} % of the disk IOPS.`)
...
_mem_check_string=Object.entries(_get_wrk_mem_func()).map(([scope,func])=>`${scope}=${bytesize_to_hr(func(request.options, response))}`).join('; ');console.info(`The working memory usage based on memory profile on all profiles are ${_mem_check_string}.`);return null;}
function _checkpoint_tune(request,response){console.info(`===== Checkpoint Tuning =====\nImpacted attributes: checkpoint_timeout, checkpoint_completion_target, checkpoint_warning`)
const managed_cache=response.get_managed_cache(_TARGET_SCOPE)
const _data_tput=request.options.data_index_spec.perf()[0]
...
if(request.options.workload_type===PG_WORKLOAD.OLAP){_shared_buffers_ratio=0.15}else if(request.options.workload_type===PG_WORKLOAD.VECTOR){_shared_buffers_ratio=0.02}else if(request.options.workload_type===PG_WORKLOAD.TSR_IOT){_shared_buffers_ratio=0.99}
...
total_ckpt_time+=Math.ceil(Math.max(32*Mi+48*Mi*request.options.workload_profile.num(),4*request.options.tuning_kwargs.wal_segment_size)/Mi*(1/_data_trans_tput+1/_wal_tput))
let after_checkpoint_timeout=realign_value(Math.max(managed_cache['checkpoint_timeout'],total_ckpt_time),Math.floor(MINUTE/2))[request.options.align_index]
...
_ApplyItmTune('checkpoint_warning',Math.floor(after_checkpoint_timeout*0.90*(1-managed_cache['checkpoint_completion_target'])),PG_SCOPE.ARCHIVE_RECOVERY_BACKUP_RESTORE,response)
return null;}
...
return null;}
function _stune_v18(request,response){if(request.options.pgsql_version<18){console.warn(`The PostgreSQL version is ${request.options.pgsql_version} < 18 -> Skip the PostgreSQL 18 tuning`)
return null;}
...
const _CalibrationProfile={[PG_WORKLOAD.OLAP]:{'cpu_to_connection_scale_ratio':2.5,'hash_mem_usage_level':-2.0,'shared_buffers_ratio':0.33,'max_work_buffer_ratio':0.175,'max_normal_memory_usage':0.60,},[PG_WORKLOAD.HTAP]:{'cpu_to_connection_scale_ratio':4.0,'hash_mem_usage_level':-2.5,'shared_buffers_ratio':0.30,'max_work_buffer_ratio':0.15,'max_normal_memory_usage':0.60,},[PG_WORKLOAD.VECTOR]:{'shared_buffers_ratio':0.33,'temp_buffers_ratio':0.125,},[PG_WORKLOAD.TSR_IOT]:{'cpu_to_connection_scale_ratio':6.0,'temp_buffers_ratio':0.20,'hash_mem_usage_level':-4.0,},}
function _AutoCalibrateProfile(){const workload_type=PG_WORKLOAD[_get_text_element(`workload_type`).toUpperCase()];if(_CalibrationProfile.hasOwnProperty(workload_type)){for(const[key,value]of Object.entries(_CalibrationProfile[workload_type])){_set_text_element(`keywords.${key}`,value);}}}