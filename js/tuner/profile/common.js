...
import { MULTI_ITEMS_SPLIT } from './js_src/static.js';

export const merge_extra_info_to_profile = (profiles) => {
    /**
     * Merge the profile data into a single file.
     */
    for (const [_, [_, items, extra_default]] of Object.entries(profiles)) {
...
export const type_validation = (profiles) => {
    /** Type validation for the profile data. */
    for (const [_, [scope, category_profile, _]] of Object.entries(profiles)) {
...
export const rewrite_items = (profiles) => {
    /** Drop the deprecated items from the profile data. */
    for (const [_, [_, items, _]] of Object.entries(profiles)) {
        const remove_keys = [];
        for (const [mkey, _] of Object.entries(items)) {
            if (mkey.startsWith('-')) {
