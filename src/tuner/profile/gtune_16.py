...
_DB_VACUUM_PROFILE = {
    'vacuum_buffer_usage_limit': {
        'instructions': {
            ...
            realign_value_to_unit(cap_value(global_cache['shared_buffers'] // 32, 2 * Mi, 16 * Gi), DB_PAGE_SIZE)[0],
            'mall': lambda group_cache, global_cache, options, response:
            realign_value_to_unit(cap_value(global_cache['shared_buffers'] // 48, 2 * Mi, 16 * Gi), DB_PAGE_SIZE)[0],
            'bigt': lambda group_cache, global_cache, options, response:
            realign_value_to_unit(cap_value(global_cache['shared_buffers'] // 64, 2 * Mi, 16 * Gi), DB_PAGE_SIZE)[0],
        },
...
DB_CONFIG_PROFILE = DB16_CONFIG_PROFILE.copy() if DB16_CONFIG_MAPPING else DB16_CONFIG_PROFILE
