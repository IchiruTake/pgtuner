...
class PG_TUNE_USR_OPTIONS{constructor(options={}){this.workload_profile=options.workload_profile??PG_SIZING.LARGE;this.pgsql_version=options.pgsql_version??17;this.data_index_spec=options.data_index_spec;this.wal_spec=options.wal_spec;this.max_backup_replication_tool=options.max_backup_replication_tool??PG_BACKUP_TOOL.PG_BASEBACKUP;this.opt_transaction_lost=options.opt_transaction_lost??PG_PROFILE_OPTMODE.NONE;this.opt_wal_buffers=options.opt_wal_buffers??PG_PROFILE_OPTMODE.SPIDEY;this.max_time_transaction_loss_allow_in_second=options.max_time_transaction_loss_allow_in_second??650;this.max_num_stream_replicas_on_standby=options.max_num_stream_replicas_on_standby??0;this.max_num_logical_replicas_on_standby=options.max_num_logical_replicas_on_standby??0;this.offshore_replication=options.offshore_replication??false;this.workload_type=options.workload_type??PG_WORKLOAD.HTAP;this.opt_mem_pool=options.opt_mem_pool??PG_PROFILE_OPTMODE.OPTIMUS_PRIME;this.tuning_kwargs=options.tuning_kwargs??new PG_TUNE_USR_KWARGS();this.database_size_in_gib=options.database_size_in_gib??0;this.num_write_transaction_per_hour_on_workload=options.num_write_transaction_per_hour_on_workload??(50*K10);this.operating_system=options.operating_system??'linux';this.vcpu=options.vcpu??4;this.total_ram=options.total_ram??(16*Gi);this.base_kernel_memory_usage=options.base_kernel_memory_usage??-1;this.base_monitoring_memory_usage=options.base_monitoring_memory_usage??-1;this.enable_database_general_tuning=options.enable_database_general_tuning??true;this.enable_database_correction_tuning=options.enable_database_correction_tuning??true;this.align_index=options.align_index??0;this.model_post_init();}
...
_DB_ASYNC_CPU_PROFILE={'max_worker_processes':{'tune_op':(group_cache,global_cache,options,response)=>cap_value(Math.ceil(options.vcpu*1.5)+2,4,512),'default':8,},'max_parallel_workers':{'tune_op':(group_cache,global_cache,options,response)=>Math.min(cap_value(Math.ceil(options.vcpu*1.125),4,512),group_cache['max_worker_processes']),'default':8,},'max_parallel_workers_per_gather':{'tune_op':(group_cache,global_cache,options,response)=>Math.min(cap_value(Math.ceil(options.vcpu/3),2,32),group_cache['max_parallel_workers']),'default':2,},'max_parallel_maintenance_workers':{'tune_op':(group_cache,global_cache,options,response)=>Math.min(cap_value(Math.ceil(options.vcpu/2),2,32),group_cache['max_parallel_workers']),'default':2,},'min_parallel_table_scan_size':{'instructions':{'medium_default':16*Mi,'large_default':24*Mi,'mall_default':32*Mi,'bigt_default':32*Mi,},'default':8*Mi,'partial_func':(value)=>`${Math.floor(value / DB_PAGE_SIZE) * Math.floor(DB_PAGE_SIZE / Ki)}kB`,},'min_parallel_index_scan_size':{'tune_op':(group_cache,global_cache,options,response)=>Math.max(group_cache['min_parallel_table_scan_size']/16,512*Ki),'default':512*Ki,'partial_func':(value)=>`${Math.floor(value / DB_PAGE_SIZE) * Math.floor(DB_PAGE_SIZE / Ki)}kB`,},}
...
class PG_TUNE_REQUEST{constructor(options){this.options=options.options||{};this.include_comment=options.include_comment||false;this.custom_style=options.custom_style||null;}}
...
if(request.options.opt_wal_buffers===PG_PROFILE_OPTMODE.NONE){request.options.opt_wal_buffers=PG_PROFILE_OPTMODE.SPIDEY
...
let transaction_loss_ratio=2/3.25
if(request.options.opt_wal_buffers===PG_PROFILE_OPTMODE.OPTIMUS_PRIME){data_amount_ratio_input=1.5
transaction_loss_ratio=3/3.25}else if(request.options.opt_wal_buffers===PG_PROFILE_OPTMODE.PRIMORDIAL){data_amount_ratio_input=2
...
options=new PG_TUNE_USR_OPTIONS({enable_sysctl_general_tuning:false,enable_sysctl_correction_tuning:false,enable_database_general_tuning:true,enable_database_correction_tuning:true,workload_profile:PG_SIZING.LARGE,pgsql_version:17,database_size_in_gib:0,num_write_transaction_per_hour_on_workload:50*K10,align_index:1,data_index_spec:data_index_disk,wal_spec:wal_disk,tuning_kwargs:kw,workload_type:PG_WORKLOAD.HTAP,operating_system:'containerd',base_kernel_memory_usage:-1,base_monitoring_memory_usage:-1,vcpu:8,total_ram:8*5*Gi,opt_mem_pool:PG_PROFILE_OPTMODE.OPTIMUS_PRIME,opt_transaction_lost:PG_PROFILE_OPTMODE.NONE,opt_wal_buffers:PG_PROFILE_OPTMODE.SPIDEY,max_time_transaction_loss_allow_in_millisecond:650,max_num_stream_replicas_on_primary:0,max_num_logical_replicas_on_primary:0,max_backup_replication_tool:PG_BACKUP_TOOL.PG_BASEBACKUP,offshore_replication:false,})
