...
let after_bgwriter_delay=Math.floor(Math.max(100,300-30*request.options.workload_profile.num()-5*data_iops/K10))
_ApplyItmTune('bgwriter_delay',after_bgwriter_delay,PG_SCOPE.OTHERS,response)
const bg_io_per_cycle=0.065
const iops_ratio=1/(1/bg_io_per_cycle-1)
const after_bgwriter_lru_maxpages=cap_value(50*request.options.workload_profile.num()+data_iops*cap_value(iops_ratio,1e-6,1e-1),100+50*request.options.workload_profile.num(),10000);_ApplyItmTune('bgwriter_lru_maxpages',after_bgwriter_lru_maxpages,PG_SCOPE.OTHERS,response);console.log(`Start tuning the autovacuum of the PostgreSQL database server based on the database workload.`)
