...
_file_config(target,request,exclude_names=null){let content=[target.disclaimer(),'\n'];if(request.backup_settings){content.push(`# User Options: ${JSON.stringify(request.options)}\n`);}
...
return content.join('');}
_generate_content_as_response(target,request,exclude_names=null){let content={};for(const[_,items]of Object.entries(this.outcome[target])){for(const[item_name,item]of Object.entries(items)){if(exclude_names===null||!exclude_names.has(item_name)){content[item_name]=item.out_display(null);}}}
...
if(request.output_format==='file'){return this._generate_content_as_file(target,request,exclude_names);}else if(['json','conf'].includes(request.output_format)){return this._generate_content_as_response(target,request,exclude_names);}else{throw new Error(`Invalid output format: ${request.output_format}. Expected one of "json", "conf", "file".`);}}
...
function _build_request_from_backend(data){return new PG_TUNE_REQUEST({'options':_build_options_from_backend(data.options),'include_comment':data.include_comment??false,'custom_style':data.custom_style??false,'backup_settings':data.backup_settings??true,'analyze_with_full_connection_use':data.analyze_with_full_connection_use??true,'ignore_non_performance_setting':data.ignore_non_performance_setting??false,'output_format':data.output_format??'file',})}
function _build_request_from_html(){return{'options':_build_options_from_html(),'include_comment':_get_checkbox_element(`include_comment`)??false,'custom_style':_get_checkbox_element(`custom_style`)??false,'backup_settings':_get_checkbox_element(`backup_settings`)??true,'analyze_with_full_connection_use':_get_checkbox_element(`analyze_with_full_connection_use`)??true,'ignore_non_performance_setting':_get_checkbox_element(`ignore_non_performance_setting`)??false,'output_format':_get_text_element(`output_format`)??'file',}}
