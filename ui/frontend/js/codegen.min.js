...
total_ckpt_time+=Math.ceil(max(32*Mi+48*Mi*request.options.workload_profile.num(),4*request.options.tuning_kwargs.wal_segment_size)*(1/_data_trans_tput+1/_wal_tput))
let after_checkpoint_timeout=realign_value(max(managed_cache['checkpoint_timeout'],total_ckpt_time),Math.floor(MINUTE/2))[request.options.align_index]
